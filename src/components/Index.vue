...
import GitSetting from "@/components/GitSetting.vue";
...
let choseId = ref(0)
let urlMapper = ref([])
let gitModal = ref(null)
let gitSetting = ref(null)
onMounted(() => {
...
const generateCode = () => {
  generateRequest.token = localStorage.getItem('token')
...
const changeSetting = () => {
  if (chose.value === 'git') {
    gitModal.value.openModal()
  }
}

const openGitSetting = () => {
  gitSetting.value.openModal()
}
...
<template>
  <div class="main" style="margin-top: 7px;">
    <div class="setBox">
      ...
      <a-button style="margin-left: 7px; margin-right: 7px; color: blue;"
                @click="openGitSetting">Git配置
      </a-button>
    </div>
    ...
    <GitModal ref="gitModal"/>

    <GitSetting ref="gitSetting"/>
  </div>
</template>
